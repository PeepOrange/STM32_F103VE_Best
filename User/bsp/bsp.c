...
{  
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_CRC, ENABLE);
	...
	Bsp_NVIC_Configuration();  /* 中断优先级分组配置为中断分组2 */
	/* LED 端口初始化 */
	LED_GPIO_Config();  
	/* 按键初始化 */
	Key1_GPIO_Config();
	...
	if(IsCal==0x55)
	{
		...
		GUI_TOUCH_Calibrate(GUI_COORD_X,readCal[0], readCal[4], readCal[6], readCal[2]); /* X轴校准参数*/
		GUI_TOUCH_Calibrate(GUI_COORD_Y,readCal[1], readCal[5], readCal[7], readCal[3]); /* Y轴校准参数 */
	}
	/************************************************************/
	/* 配置RTC秒中断优先级 */
	RTC_NVIC_Config();
